...
describe("mapModel", () => {
  test("maps any Anthropic model to composer-2.5 (no haiku->fast cost trap)", () => {
    expect(mapModel("claude-3-5-haiku-20241022")).toBe("composer-2.5");
    expect(mapModel("claude-sonnet-4-20250514")).toBe("composer-2.5");
    expect(mapModel(undefined)).toBe("composer-2.5");
  });
});
