...
This is the SINGLE SOURCE OF TRUTH for the `windows-app/` Tauri 2 port. Every value here was
verified against the real `macos/CursorAPI` Swift sources and `worker/` TypeScript. Where the
...
- We create everything under `windows-app/` plus additive edits to `.github/workflows/release-windows.yml`,
...
## 4. Sidecar (windows-app/sidecar/server.ts)
- A standalone `node:http` (or `Bun.serve`) server. **Reuse the pure worker helpers** by importing from
  the repo `worker/` modules (relative path from `windows-app/sidecar/`: `../../worker/openai`, `../../worker/cursor`,
...
- Build target name (Tauri sidecar): base name `api-for-cursor-server`; release bundles
  `windows-app/src-tauri/binaries/api-for-cursor-server-x86_64-pc-windows-msvc.exe` via
...
- release-windows.yml: trigger on tags `v*.*.*-win` + workflow_dispatch; runner windows-latest; working-directory windows-app; setup node 22 + rust msvc + bun; `bun install`; build sidecar via `bun build`; `bun tauri build`; sign NSIS with signtool (gated on cert secret present); upload artifact; publish to R2 via `windows-app/scripts/publish-windows.ts` (gated on R2 secrets). All signing/publish steps must be conditional so the build still succeeds without secrets.
...
- `windows-app/src-tauri` → `cargo check` passes (all modules compile).
- `windows-app` frontend → `bun install` + `bun run build` (vite + tsc) passes.
- `windows-app/sidecar` → `bun build sidecar/server.ts --compile` produces an exe; running it serves `GET /v1/models` (HTTP 200 JSON list with composer-2.5) and `/health` with NO API key.
