# API for Cursor — Windows app

A native **Windows desktop app** that runs a local **OpenAI-compatible API** backed by
**Cursor's Composer models**, so any OpenAI-compatible coding agent (OpenCode, Codex,
Cline, Kilo Code, pi, VS Code, …) can use Cursor through `http://127.0.0.1:8787/v1`.

It is the Windows port of the macOS "API for Cursor" menu-bar app, built with
[Tauri 2](https://v2.tauri.app/) (Rust + WebView2 + a React UI). It lives in the system
tray, stores your Cursor API key in the **Windows Credential Manager**, and supports
one-click setup for popular agents and in-app auto-update.
...
> **Claude Code is not supported.** It speaks Anthropic's Messages API; this server is
> OpenAI-compatible (`/v1/chat/completions`). Use an OpenAI-compatible agent, or a separate
...
# 1. Compile the main API-server sidecar.
...
> The build depends on sibling source in the repo (`../worker/*.ts`, `../scripts/cursor-sdk-local-agent-bridge.mjs`).
> Build from a full checkout, not `windows-app/` in isolation.
